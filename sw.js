...
///
/// Instead the cache name carries the build, and `21f5eacbcbf9` is replaced with
...
///
const CACHE = 'soundas-21f5eacbcbf9';

/// The graphics engine, kept apart from the app.
///
/// CanvasKit is 2.8 MB of the 3.8 MB a first visit costs, and it changes only
/// when the Flutter SDK does — never because the app was rebuilt. Holding it in
/// the build-stamped cache meant every release threw it away with everything
/// else, so a user who had already downloaded it did so again for each release:
/// the site was quick, then slow again, then quick.
///
/// Stamped with the SDK instead, so an app release leaves it alone and only a
/// Flutter upgrade replaces it.
const ENGINE_CACHE = 'soundas-engine-3.44.4';

/// Which cache a request belongs in. Everything the engine ships under one
/// folder, which is the whole of what outlives a release.
const isEngine = (url) => new URL(url).pathname.includes('/canvaskit/');

/// Caching is switched off entirely when the app is served locally.
///
/// `21f5eacbcbf9` is only ever replaced at deploy time, so a local run has one
...
// The app bundle is deliberately not in here. The build ships two of them —
// main.dart.wasm for browsers with WasmGC, main.dart.js for the rest — and only
// one is ever used. Naming either would have the worker download a megabyte the
// browser has no use for. Whichever the page actually fetches is cached on the
// way past, which is what makes the next launch offline anyway.
  ...
  event.waitUntil(
    (async () => {
      ...
      await Promise.all(
        names
            .filter((n) => n !== INBOX)
            .filter((n) => LOCAL || (n !== CACHE && n !== ENGINE_CACHE))
...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(
    (async () => {
      ...
      // The engine keeps its own cache so a release does not evict it.
      const target = isEngine(request.url)
        ? await caches.open(ENGINE_CACHE)
        : cache;

      const hit = await target.match(request);
      if (hit) return hit;

      const res = await fetch(request);
      if (res.ok && res.status === 200) {
        target.put(request, res.clone()).catch(() => {});
      }
      return res;
    })(),
  );
